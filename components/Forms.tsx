...
const InputField = ({ label, type = "text", placeholder = "", required = false }: any) => (
  <div className="space-y-1.5">
    ...
    <input 
      ...
      className="w-full px-4 py-3 bg-slate-50 border-none rounded-xl text-slate-700 text-sm focus:ring-2 focus:ring-blue-100 transition-all placeholder:text-slate-300"
    />
  </div>
);
...
const SelectField = ({ label, options, required = false }: any) => (
  <div className="space-y-1.5">
    <label className="text-xs font-bold text-slate-400 uppercase tracking-wider">{label} {required && '*'}</label>
    <select className="w-full px-4 py-3 bg-slate-50 border-none rounded-xl text-slate-700 text-sm focus:ring-2 focus:ring-blue-100 transition-all appearance-none cursor-pointer">
...
const TextareaField = ({ label, placeholder = "", rows = 3 }: any) => (
  <div className="space-y-1.5">
    ...
    <textarea 
      rows={rows}
      placeholder={placeholder}
      className="w-full px-4 py-3 bg-slate-50 border-none rounded-xl text-slate-700 text-sm focus:ring-2 focus:ring-blue-100 transition-all placeholder:text-slate-300 resize-none"
    />
  </div>
);
...
const FormActions = ({ onCancel }: { onCancel: () => void }) => (
  <div className="flex gap-3 pt-6 mt-6 border-t border-slate-50">
    ...
    <button 
      type="submit"
      className="flex-[2] py-3 bg-blue-600 text-white font-bold text-sm rounded-xl shadow-lg shadow-blue-100 hover:bg-blue-700 transition-all active:scale-[0.98]"
    >
      บันทึกข้อมูล
    </button>
  </div>
...
export const ReportForm: React.FC<FormProps> = ({ onCancel, onSubmit }) => (
  <form onSubmit={(e) => { e.preventDefault(); onSubmit({}); }} className="space-y-5">
    <div className="grid grid-cols-2 gap-4">
      ...
      <SelectField label="ความรุนแรงของปัญหา" options={Object.values(IssueLevel)} />
    </div>
    <TextareaField label="ปัญหาที่พบหน้างาน" placeholder="อธิบายปัญหาที่พบ หรือความคืบหน้าที่ต้องการรายงาน..." required />
    <div className="space-y-1.5">
      <label className="text-xs font-bold text-slate-400 uppercase tracking-wider">รูปถ่ายหน้างาน</label>
      <div className="w-full h-32 border-2 border-dashed border-slate-200 rounded-2xl flex flex-col items-center justify-center text-slate-400 hover:bg-slate-50 transition-colors cursor-pointer group">
         <div className="p-3 bg-white rounded-full shadow-sm group-hover:scale-110 transition-transform mb-2">📸</div>
         <span className="text-xs font-bold">อัปโหลดรูปภาพ</span>
      </div>
    </div>
    <FormActions onCancel={onCancel} />
  </form>
...
export const AppointmentForm: React.FC<FormProps> = ({ onCancel, onSubmit }) => (
  <form onSubmit={(e) => { e.preventDefault(); onSubmit({}); }} className="space-y-5">
    ...
    <SelectField label="โครงการที่เกี่ยวข้อง" options={['แกรนด์ เรสซิเดนซ์', 'อีโค ทาวเวอร์', 'งานส่วนกลาง', 'อื่นๆ']} />
